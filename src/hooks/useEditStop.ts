...
import {
  ...
  GetStopRoutesByIdDocument,
  GetStopRoutesByIdQuery,
  GetStopRoutesByIdQueryVariables,
...
import { mapGetStopResult, mapGetStopRoutesByIdResult } from '../graphql';
...
export const useEditStop = () => {
  ...
  const isStartingOrEndingStopOfAnyRoute = (
    stopId: UUID,
    stopWithRoutes?: ServicePatternScheduledStopPoint,
  ) => {
    return stopWithRoutes?.scheduled_stop_point_in_journey_patterns.some(
      (item) => {
  ...
  const getJourneyPatternsToDeleteStopFrom = (
    ...
    return (
      stopWithRoutes.scheduled_stop_point_in_journey_patterns
  ...
  const prepareAndValidateEdit = async ({ stopId, patch }: EditParams) => {
    ...
    const oldStop = mapGetStopResult(stopResult);
    ...
    if (newLocation && !isEqual(newLocation, oldLocation)) {
      ...
      const stopWithRoutes = mapGetStopRoutesByIdResult(stopRoutesResult);
      if (isStartingOrEndingStopOfAnyRoute(stopId, stopWithRoutes)) {
      ...
      changes.deleteStopFromJourneyPatterns =
        getJourneyPatternsToDeleteStopFrom(closestLinkId, stopWithRoutes);
