...
export const useEditStop = () => {
  ...
  const onStopLocationChanged = async (
    ...
    const brokenRoutesResult = await getBrokenRoutes({
      ...
      new_label: newStop.label || oldStop.label,
    ...
    const deleteStopFromJourneyPatternIds = brokenRoutes
      ? brokenRoutes?.map((route) => route.journey_pattern_id)
      : [];
    const deleteStopFromRoutes = brokenRoutes
      ? brokenRoutes?.map((route) => route.journey_pattern_route as RouteRoute)
      : [];
