...
export const useExtractRouteFromFeature = () => {
  ...
  const getOldInfraLinksAndStops = useCallback(
    (
      ...
      const oldStopIds =
        mapRouteStopsToStopIds(stateStops) || creatingNewRoute
          ? []
          : routes.flatMap((route) => getRouteStopIds(route));

      const oldInfraLinks =
        stateInfraLinks || creatingNewRoute
          ? []
          : mapGraphQLRouteToInfraLinks(routes[0]);

      return { oldStopIds, oldInfraLinks };
    },
    [],
  );
