...
export const usePagination = (): {
  currentPage: number;
  getPaginatedData: <T>(data: Array<T>, contentPerPage: number) => Array<T>;
  setPage: (page: number) => void;
  getRenderedPageNumber: (page: number) => number | string;
  ...
  const getPaginatedData = <T>(data: Array<T>, contentPerPage: number) => {
    const currentPage = initialPage;
    return data?.slice(
      currentPage * contentPerPage - contentPerPage,
  ...
  const getSliceIndexes = (
    ...
    // If we are near start or near end, we render more page numbers to
