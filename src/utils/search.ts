...
export type SearchParametersGqlConfigurations = {
  priority: {
    value: Priority[];
    replaceStar: boolean;
    operator: string;
...
const setConfigurations = (queryParams: SearchConditions) => {
  const params: SearchParametersGqlConfigurations = {
    priority: {
      ...defaultSearchParametersGqlConfig.priority,
      value: queryParams.priorities,
    },
    label: {
      ...defaultSearchParametersGqlConfig.label,
...
/**
 * Constructs GQL filter object from the search conditions.
 * @param params
 */
...
/**
 * Creates query string from the search parameters
 * @param paramObject
 */
