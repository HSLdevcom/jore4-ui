...
describe(`<${Pagination.name}>`, () => {
  test('should render all 5 page numbers', async () => {
    render(<Pagination contentPerPage={2} totalCount={10} />);
  ...
  test('prev and next button should be disabled if only one page', async () => {
    urlQueryMock.mockReturnValueOnce({ page: 1 });
    render(<Pagination contentPerPage={10} totalCount={10} />);
  ...
  test('should render dots next to first page butotn', async () => {
    urlQueryMock.mockReturnValueOnce({ page: 8 });
    render(
      <Pagination amountOfNeighbours={1} contentPerPage={2} totalCount={20} />,
    );
