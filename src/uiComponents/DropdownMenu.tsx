...
export const DropdownMenu = ({ children, testId }: Props): JSX.Element => {
  return (
      ...
      {({ open }) => (
        <>
          <Menu.Button
            ...
            data-testid={testId}
          >
            <i className="icon-more" style={{ fontSize: 14 }} />
          </Menu.Button>
                ...
                <Menu.Item>
                  {() =>
                    React.isValidElement(child)
                      ? addClassName(
                          child,
                          `border-x border-b first-of-type:border-t border-black w-full py-1 px-2 focus:outline-none`,
                        )
                      : child
                  }
                </Menu.Item>
