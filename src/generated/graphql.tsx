...
export type GetActiveLineDetailsWithRoutesByDateQueryVariables = Exact<{
  ...
  filterPriority?: Maybe<Array<Scalars['Int']> | Scalars['Int']>;
}>;
...
export const GetActiveLineDetailsWithRoutesByDateDocument = gql`
    query GetActiveLineDetailsWithRoutesByDate($label: String!, $date: timestamptz, $filterPriority: [Int!]) {
  route_line(
    where: {label: {_eq: $label}, _and: [{_or: [{validity_start: {_lte: $date}}, {validity_start: {_is_null: true}}]}, {_or: [{validity_end: {_gte: $date}}, {validity_end: {_is_null: true}}]}], priority: {_nin: $filterPriority}}
    order_by: {priority: desc}
    limit: 1
  ) {
    ...line_all_fields
    line_routes(
      where: {_and: [{_or: [{validity_start: {_lte: $date}}, {validity_start: {_is_null: true}}]}, {_or: [{validity_end: {_gte: $date}}, {validity_end: {_is_null: true}}]}], priority: {_nin: $filterPriority}}
    ) {
      ...route_with_stops
      infrastructure_links_along_route {
        infrastructure_link {
          scheduled_stop_point_located_on_infrastructure_link(
            where: {_and: [{_or: [{validity_start: {_lte: $date}}, {validity_start: {_is_null: true}}]}, {_or: [{validity_end: {_gte: $date}}, {validity_end: {_is_null: true}}]}]}
          ) {
            ...scheduled_stop_point_default_fields
            scheduled_stop_point_in_journey_patterns {
              ...scheduled_stop_point_in_journey_pattern_default_fields
