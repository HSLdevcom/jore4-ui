...
export const Stops = React.forwardRef((props, ref) => {
  ...
  const [unsavedStop, setUnsavedStop] =
  ...
  useImperativeHandle(ref, () => ({
    onCreateStop: (e: MapEvent) => {
      const draftStop: Partial<ServicePatternScheduledStopPoint> = {
        measured_location: {
          coordinates: e.lngLat,
        },
      };

      setUnsavedStop(draftStop);
      onOpenPopup(draftStop);
    },
  }));
    ...
    <>
      ...
      {unsavedStop && (
        <Stop
          selected
          longitude={
            mapLngLatToPoint(unsavedStop.measured_location.coordinates)
              .longitude
          }
          latitude={
            mapLngLatToPoint(unsavedStop.measured_location.coordinates).latitude
          }
          onClick={() => onOpenPopup(unsavedStop)}
          onDragEnd={() => null}
        />
      )}
      {popupInfo && (
