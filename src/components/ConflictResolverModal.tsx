...
import { GetLinesByValidityQuery } from '../generated/graphql';
import { Row } from '../layoutComponents';
import { mapToShortDate } from '../time';
import { Priority } from '../types/Priority';
import { CloseIconButton, Modal, SimpleButton } from '../uiComponents';
import { mapPriorityToUiName } from '../utils';
...
export const mapLinesToCommonConflictItems = (
  lines: GetLinesByValidityQuery['route_line'],
): CommonConflictItem[] =>
  lines.map((item) => ({
    validityStart: item.validity_start,
    validityEnd: item.validity_end,
    priority: item.priority,
    label: item.label,
    id: item.line_id,
    href: `/lines/${item.line_id}`,
  }));
...
export const ConflictResolverModal: React.FC<Props> = ({
  ...
  const renderConflictRow = (item: CommonConflictItem) => (
    <tr key={item.id}>
  ...
  return (
    ...
    >
      ...
      <table className="mt-6">
        ...
        <tbody>{conflicts.map(renderConflictRow)}</tbody>
      </table>
