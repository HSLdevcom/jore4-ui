...
const StopFormComponent = (
  ...
  const {
    prepareAndValidate: prepareEdit,
    editStopMutation,
    mapChangesToMutationVariables: mapToEditVariables,
    defaultErrorHandler,
  } = useEditStop();
  const {
    prepareAndValidate: prepareCreate,
    mapChangesToMutationVariables: mapToCreateVariables,
    insertStopMutation,
  } = useCreateStop();
  ...
  const onEdit = async (state: FormState) => {
    try {
      ...
      const variables = mapToEditVariables(changes);
  ...
  const onCreate = async (state: FormState) => {
    try {
      ...
      const variables = mapToCreateVariables(changes);
