...
// until following issue is resolved. Otherwise editing won't workconsole.log;
...
const DrawRouteLayerComponent = (
  ...
  const onAddRoute = async (e: EditorCallback) => {
    const lastFeature = e.data.length - 1;
    const routeId = String(lastFeature);
    const { coordinates } = e.data[lastFeature].geometry;
  ...
  return (
    <Editor
      ...
      clickRadius={12}
      mode={modeHandler}
      // Editor calls onUpdate callback million times when route is being edited. This we want to debounce the event.
