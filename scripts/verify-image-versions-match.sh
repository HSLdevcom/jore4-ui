...
do
  ...
  image1=$(grep $item docker/docker-compose.custom.yml | grep 'image:' | sed -r "s/^[[:space:]]*image:[[:space:]]*['\"]${item//\//\\/}--(.*)['\"]/\1/")

  # Find version in e2e yml
  image2=$(grep $item docker/docker-compose.e2e.yml | grep 'image:' | sed -r "s/^[[:space:]]*image:[[:space:]]*['\"]${item//\//\\/}--(.*)['\"]/\1/")

  if test "$image1" != "$image2"; then
