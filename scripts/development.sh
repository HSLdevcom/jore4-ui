...
function import_dump {
  echo "Importing JORE4 dumps to databases"
...
function lolling {
  ...
  # Download dumps if any of them are missing
  if [ ! -f $DUMP_ROUTES_FILENAME ]; then
    download_dump $DUMP_ROUTES_FILENAME
  fi
  if [ ! -f $DUMP_TIMETABLES_FILENAME ]; then
    download_dump $DUMP_TIMETABLES_FILENAME
  fi
  if [ ! -f $DUMP_STOPS_FILENAME ]; then
    download_dump $DUMP_STOPS_FILENAME
  fi

  docker exec -i testdb pg_restore -U dbadmin --dbname=jore4e2e --format=c < $DUMP_ROUTES_FILENAME
  docker exec -i testdb pg_restore -U dbadmin --dbname=timetablesdb --format=c < $DUMP_TIMETABLES_FILENAME
  docker exec -i testdb pg_restore -U dbadmin --dbname=stopdb --format=c < $DUMP_STOPS_FILENAME
}
