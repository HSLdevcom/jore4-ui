...
const seedDb = async () => {
  ...
  const vsf1 = buildVehicleScheduleFrameDeep({
    vsfBase: {
      ...
      validity_start: DateTime.now().minus({ months: 3 }),
      validity_end: DateTime.now().plus({ months: 6 }),
    },
    vsByDay: {
      [MON_FRI_DAY_TYPE]: {
        ...defaultTptSeqParams,
  ...
  const vsf2 = buildVehicleScheduleFrameDeep({
    vsfBase: {
      ...
      validity_start: DateTime.now().minus({ weeks: 1 }),
      validity_end: DateTime.now().plus({ weeks: 3 }),
    },
    vsByDay: {
      [SAT_DAY_TYPE]: {
        ...defaultTptSeqParams,
