...
interface TimetabledPassingTimesForJourney {
  ...
  startTime: Duration; // when does journey start
...
const buildTimetabledPassingTimesForJourney = ({
  ...
  return scheduledStopLabels.map((item, index) => {
    const stopId = findStopIdByLabel(item);

    const travelTimeFromStart = multiplyDuration(stopInterval, index);
    const stopTimeFromStart = multiplyDuration(waitTimeOnStops, index);
    const durationFromStart = travelTimeFromStart.plus(stopTimeFromStart);

    const timeAtArrival = startTime.plus(durationFromStart);
    const timeAtDeparture = timeAtArrival.plus(waitTimeOnStops);

    const waitsAtStop = !timeAtArrival.equals(timeAtDeparture);
    const isLastStop = index + 1 === scheduledStopLabels.length;

    // arrival time is set to null if is it same as departure time
    const arrivalTime = waitsAtStop || isLastStop ? timeAtArrival : null;
    const departureTime = isLastStop ? null : timeAtDeparture;

    return buildTimetabledPassingTime({
      vehicleJourneyId,
      stopInJourneyPatternRefId: stopId,
      arrivalTime,
      departureTime,
    });
  });
...
const buildBulkJourneys = ({
  vehicleJourneyIds,
  startTime,
  startInterval = Duration.fromISO('PT5M'),
}: {
  ...
  return vehicleJourneyIds
    .map((item, index) => {
      const durationFromStart = multiplyDuration(startInterval, index + 1);
      return buildTimetabledPassingTimesForJourney({
        vehicleJourneyId: item,
        startTime: startTime.plus(durationFromStart),
      });
    })
    .flat();
};
...
const seedTimetabledPassingTimesMonFri: TimetabledPassingTimeInsertInput[] = [
  ...
  ...buildTimetabledPassingTimesForJourney({
    ...
    startTime: Duration.fromISO('PT8H5M'),
  ...
  ...buildTimetabledPassingTimesForJourney({
    ...
    startTime: Duration.fromISO('PT7H38M'),
    waitTimeOnStops: Duration.fromISO('PT2M'),
  }),
  ...
  ...buildTimetabledPassingTimesForJourney({
    ...
    startTime: Duration.fromISO('PT9H5M'),
  ...
  ...buildTimetabledPassingTimesForJourney({
    ...
    startTime: Duration.fromISO('PT7H10M'),
  ...
  ...buildBulkJourneys({
    ...
    startTime: Duration.fromISO('PT7H18M'),
    startInterval: Duration.fromISO('PT2M'),
  }),
];
...
const seedTimetabledPassingTimesSat: TimetabledPassingTimeInsertInput[] = [
  ...buildBulkJourneys({
    ...
    startTime: Duration.fromISO('PT10H0M'),
  }),
];
...
const seedTimetabledPassingTimesSun: TimetabledPassingTimeInsertInput[] = [
  ...buildTimetabledPassingTimesForJourney({
    ...
    startTime: Duration.fromISO('PT10H15M'),
    stopInterval: Duration.fromISO('PT15M'),
  }),
];
