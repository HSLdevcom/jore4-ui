...
export const findEarliestDate = (values: FormState) => {
  const period = values.periods.reduce((prev, curr) => {
    return DateTime.fromISO(prev.beginDate) < DateTime.fromISO(curr.beginDate)
      ? prev
      : curr;
  });
  return DateTime.fromISO(period.beginDate);
};

export const findLatestDate = (values: FormState) => {
  const period = values.periods.reduce((prev, curr) => {
    return DateTime.fromISO(prev.endDate) > DateTime.fromISO(curr.endDate)
      ? prev
      : curr;
  });
  return DateTime.fromISO(period.endDate);
};
