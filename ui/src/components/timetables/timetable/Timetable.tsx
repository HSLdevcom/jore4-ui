...
import { MockTimetableRowData, TimetableRow } from './TimetableRow';
...
export const Timetable = ({ className = '' }: Props): JSX.Element => {
  const mockContent: MockTimetableRowData[] = [
    ...
    <div>
      <Row className="mb-4 rounded-md bg-hsl-dark-green bg-opacity-50 px-4 py-1">
        ...
        <Column className="justify-center">
          <p className="text-hsl-dark-80 text-sm">
      ...
      <table className={`${className}`} data-testid={testIds.timetable}>
        {mockContent.map((item) => (
          <TimetableRow key={item.hours} data={item} />
        ))}
      </table>
    </div>
  );
