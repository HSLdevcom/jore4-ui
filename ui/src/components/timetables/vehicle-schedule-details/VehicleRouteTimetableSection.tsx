...
export const VehicleRouteTimetableSection = ({
  ...
  const { activeView, showPassingTimesByStop } = useTimetablesViewState();
      ...
      <Visible visible={isOpen}>
        <div className="mt-8">
            ...
            <div className="grid grid-cols-3 gap-x-8 gap-y-5">
              {timetables?.vehicleJourneyGroups.map((item) => (
                <VehicleServiceTable
                  ...
                  onClick={() =>
                    showPassingTimesByStop(route.label, item.dayType.label)
                  }
                />
              ))}
            </div>
          ...
          {activeView === TimetablesView.PASSING_TIMES_BY_STOP &&
            timetables && <PassingTimesByStopSection timetables={timetables} />}
        </div>
