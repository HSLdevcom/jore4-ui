...
export const ExportToolbar = (): JSX.Element => {
  ...
  const exportData =
    resultType === DisplayedSearchResultType.Routes
      ...
      : {
          resultsByType: lines.filter((line) => !!line.line_routes.length),
          // lines without routes are not selectable and need to be filtered out
          searchResultLabels: lines
            .filter((line) => !!line.line_routes.length)
            .map((line) => line.label),
