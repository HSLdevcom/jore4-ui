...
function mapRouteToVersionInfoItem(
  ...
  const lineName = buildLineName(
    rawRoute.route_line.label,
    defaultLocalizedString(rawRoute.origin_name_i18n).fi_FI,
    defaultLocalizedString(rawRoute.destination_name_i18n).fi_FI,
  );
  ...
  return {
    ...
    changedByUserName: routeChangeHistory?.changed_by ?? null,
...
export function useGetRouteVersions(
  ...
  const routeVersions = useMemo(() => {
    ...
    return rawRoutes.map((rawRoute) =>
      mapRouteToVersionInfoItem(rawRoute, activeVersionId),
    );
  }, [result.data]);
