...
function serializeInfoSpots(value: StopSearchFilters['infoSpots']): string {
  if (value.noInfoSpots) {
    return NO_INFO_SPOTS;
  }
...
function parseInfoSpots(value: string): StopSearchFilters['infoSpots'] {
  if (value === NO_INFO_SPOTS) {
    return { noInfoSpots: true, sizes: [] };
  }

  const sizes = splitString(value).map((size) => {
  ...
  return { noInfoSpots: false, sizes };
}
