...
export const StopChangeHistoryPage: FC = () => {
  ...
  const [{ from, to }, setFilters] = useState<StopChangeHistoryFilters>(() => ({
  ...
  const { historyItems, loading } = useGetStopChangeHistoryItems({
    publicCode,
    from,
    to,
  ...
  return (
    <Container testId={testIds.container}>
      ...
      <LoadingWrapper
        ...
        loadingText={t('stopChangeHistory.loading')}
        loading={loading}
