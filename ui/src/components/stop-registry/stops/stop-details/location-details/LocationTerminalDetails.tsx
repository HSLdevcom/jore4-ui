...
import {
  getMemberStops,
  getMemberStopsTotal,
} from '../../../terminals/components/location-details/LocationDetailsViewCard';
...
function getTerminalDetails(stop: StopWithDetails) {
  ...
  const memberStops = enrichedTerminal ? getMemberStops(enrichedTerminal) : '-';
  const memberStopsTotal = enrichedTerminal
    ? getMemberStopsTotal(enrichedTerminal)
    : '0';
