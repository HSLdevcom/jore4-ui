...
function sortByPublicCode(stops: SelectedStop[]): SelectedStop[] {
...
function findNewlySelectedStop(
  ...
  return newValue.find(
    (item) =>
      !currentValue.some(
        (existing) =>
          existing.id === item.id && existing.quayId === item.quayId,
      ),
  );
}
...
function findDeletedStop(
  ...
  return currentValue.find(
    (item) =>
      !newValue.some(
        (remaining) =>
          remaining.id === item.id && remaining.quayId === item.quayId,
      ),
  );
}
...
function getRelatedQuays(
  ...
  currentSelection: readonly SelectedStop[],
  options: SelectedStop[],
): SelectedStop[] {
  return options.filter((option) => {
    const isSameStopPlace = option.id === selectedStop.id;
...
function processSelection(
  ...
  if (isRemovingStops) {
    ...
    return newValue.filter((stop) => stop.id !== deletedStop.id);
  }
...
export const SelectMemberStopsDropdown: FC<SelectMemberStopsDropdownProps> = ({
  ...
  const handleDropdownClose = (div: HTMLDivElement | null) => {
    if (!div) {
      return;
    }
  ...
  return (
    <HUICombobox
      ...
      value={value}
      ref={handleDropdownClose}
      data-testid={testId}
    >
      {hasNoSelectedStops && (
        <div>
          <MdWarning className="mr-2 inline text-lg text-hsl-red" />
          <span className="text-hsl-red" data-testid={testIds.warningText}>
