...
const testIds = {
  ...
  memberStopAreas: 'LocationDetailsViewCard::memberStopAreas',
  memberPlatforms: 'LocationDetailsViewCard::memberPlatforms',
};

const getMemberStopAreas = (terminal: EnrichedParentStopPlace): string => {
  const quayCodes =
    terminal.children
...
export const LocationDetailsView: FC<TerminalComponentProps> = ({
  ...
  const memberStopAreas = getMemberStopAreas(terminal);
  ...
  return (
    <div data-testid={testIds.container}>
      ...
      <DetailRow>
        <LabeledDetail
          title={t('terminalDetails.location.memberStopAreas', {
            count: terminal.children?.length ?? 0,
          })}
          detail={memberStopAreas}
          testId={testIds.memberStopAreas}
        />
        <LabeledDetail
