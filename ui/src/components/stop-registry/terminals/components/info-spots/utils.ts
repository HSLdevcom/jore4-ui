...
export const getTerminalInfoSpotLocation = (
  ...
  if (infoSpot.geometry?.coordinates) {
    return mapLngLatToPoint(infoSpot.geometry.coordinates);
  }

  const infoSpotQuay = resolveInfoSpotQuay(infoSpot, terminal);

  if (infoSpotQuay?.geometry?.coordinates) {
    return mapLngLatToPoint(infoSpotQuay.geometry.coordinates);
  }

  if (terminal.geometry?.coordinates) {
    return mapLngLatToPoint(terminal.geometry?.coordinates);
  }
