...
import { RequiredKeys, RequiredNonNullableKeys } from '../../../../types';
...
function parseResult(
  data: GetStopInfoForEditingOnMapQuery | undefined,
  quayNetexId: string,
  ...
  return { formState, timingPlaceInfo };
}
...
export function useGetStopInfoForEditingOnMap(
  ...
  const { data, ...rest } = useGetStopInfoForEditingOnMapQuery(
    quayNetexId ? { variables: { quayNetexId } } : { skip: true },
  );
