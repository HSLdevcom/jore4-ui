  ...
  const cacheDefinition = new InMemoryCache({
    typePolicies: {
      ...
      stop_registry_externalLink: {
        keyFields: (object) => {
          const parentId = object.quayId ?? object.stopPlaceId;
          return parentId ? `${parentId}:${object.orderNum}` : false;
        },
      },
      ...
      stops_database_stop_place_quays: {
        keyFields: ['stop_place_id', 'quays_id'],
      },
