...
import { useOnCloseWithReturnToQueryParam } from './useVersionsOnCloseWithReturnToQueryParam';
 ...
 */
export const useTimetableVersionsOnClose = () => {
  const { queryParams } = useUrlQuery();
  const { getUrlWithReturnToQueryString, onClose } =
    useOnCloseWithReturnToQueryParam();
