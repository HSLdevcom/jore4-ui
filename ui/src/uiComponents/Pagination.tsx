...
export const Pagination = ({
  ...
  return (
    <nav role="navigation" aria-label={`${t('accessibility:common.paginationNavigation')}`} className={`flex justify-evenly ${className}`}>
      <IconButton
      ...
      <ul className={`flex flex-1 justify-evenly`}>
        {displayedPageNumbers.map((pageNumber) =>
          pageNumber !== currentPage ? (
            ...
            <li className={`inline-block ${commonClassName} ${currentPageClassName}`}><span
              key={pageNumber}
              aria-label={`${t('accessibility:common.currentPage')} ${pageNumber}`}
              aria-current="true"
            >
              {getRenderedPageNumber(pageNumber)}
            </span></li>
