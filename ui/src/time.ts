...
export const timePartToString = (timePart: number) =>
  `${timePart >= 10 ? timePart : `0${timePart}`}`;

export const durationToShortTime = (duration: Duration) =>
  `${timePartToString(duration.hours)}:${timePartToString(duration.minutes)}`;
