...
import { getObjectKeys, isPlainObject, PlainObject } from '../../utils';
...
export type SerializerFunction<
  ...
  TValue = ValueOf<T>,
> = (key: KeyOf<T>, value: TValue, parentObject: T) => TValue | string;

export type DeserializerFunction<
  T extends PlainObject = PlainObject,
  TValue = ValueOf<T>,
> = (key: KeyOf<T>, value: TValue, parentObject: T) => TValue | SerializedTypes;
...
export function mapToStoreType<T extends PlainObject>(
  ...
  return getObjectKeys(input).reduce<StoreType<T>>((result, key) => {
    ...
    if (isArray(serializedValue)) {
      return {
        ...
        [key]: serializedValue.map((item) =>
          mapToStoreType(item, serializerFunc),
        ),
      };
    }
...
export function mapFromStoreType<T extends PlainObject>(
  ...
  return getObjectKeys(input).reduce<T>((result, key) => {
    const value = input[key];
    ...
    if (isArray(value)) {
      return {
        ...result,
        [key]: value.map((item) => mapFromStoreType(item, deserializerFunc)),
      };
    }
