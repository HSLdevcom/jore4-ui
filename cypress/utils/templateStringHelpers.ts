...
function serializationHelper(key: string, value: unknown): unknown {
  if (value === undefined) {
    return '<<<original value was undefined and replaced with this string>>>';
  }
...
function expressionToString(expression: unknown): string {
  if (typeof expression === 'object' && expression) {
    if (!objectToStringCache.has(expression)) {
      objectToStringCache.set(
        expression,
        JSON.stringify(expression, serializationHelper, 2),
      );
    }
