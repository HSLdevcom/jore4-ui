...
export class RouteTimetablesSection {
  ...
  getRouteTimetableSectionByDayType(
    ...
    return this.getRouteTimetableSection(routeLabel, routeDirection).within(
      () => {
        this.vehicleServiceTable.getHeadingButton(dayType);
      },
    );
  }

  assertRouteHasNoSchedules(routeLabel: string) {
    return cy.getByTestId(`RouteTimetablesSection::noSchedules::${routeLabel}`);
  }
}
