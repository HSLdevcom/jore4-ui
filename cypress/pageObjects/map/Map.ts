...
export class Map {
   ...
   */
  static clickAtCoordinates(longitude: number, latitude: number) {
    cy.window().then((win) => {
  ...
  static clickAtPositionFromMapMarkerByTestId(
    clickPoint: ClickPointNearMapMarker,
  ) {
  ...
  static clickAtPosition(x: number, y: number) {
    cy.getByTestId('mapPage').click(x, y);
  }

  static getNthSnappingPointHandle(nth: number) {
    return cy.get(`rect[data-index="${nth}"]`).first();
  }

  static clickNthSnappingPointHandle(nth: number) {
    Map.getNthSnappingPointHandle(nth).click();
  }

  static clickRelativePoint(xPercentage: number, yPercentage: number) {
  ...
  static getStopByStopLabelAndPriority(
    testStopLabel: string,
    priority: Priority,
  ) {
  ...
  static getStopAreaById(id: string) {
  ...
  static visit(params?: { zoom?: number; lat: number; lng: number }) {
    if (params) {
      cy.visit(
        `/map?${qs.stringify({
  ...
  static waitForLoadToComplete() {
    return Map.getLoader().should('not.exist');
  }
  ...
  static moveRouteEditorHandleByCoordinates(coordinates: {
    ...
    cy.window().then((win) => {
      ...
      Map.getMapPage().trigger('mousedown', {
        button: 1,
        buttons: 1,
        x: startPixels.x,
        y: startPixels.y,
      ...
      Map.getMapPage().trigger('mousemove', {
        button: 1,
        buttons: 1,
        x: destPixels.x,
        y: destPixels.y,
      ...
      Map.getMapPage().trigger('mouseup', {
        button: 1,
        buttons: 1,
        x: destPixels.x,
        y: destPixels.y,
