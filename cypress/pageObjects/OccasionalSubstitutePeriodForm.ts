...
export class OccasionalSubstitutePeriodForm {
  ...
  openOccasionalSubstitutePeriodForm() {
    ...
    // Form sometimes closes/disappears so we have to check if it is visible
    // and click the add button again if it is not
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(500);
    cy.get('body').then((body) => {
      if (
        body.find('[data-testid="OccasionalSubstitutePeriodRow::periodName"]')
          .length === 0
      ) {
        this.getAddOccasionalSubstitutePeriodButton().click();
      }
  ...
  fillOccasionalSubstitutePeriodForm(
    ...
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(500);
    this.getOccasionalSubstitutePeriodFormPeriodName().type(values.name);
    ...
    this.getOccasionalSubstitutePeriodFormEndDate().type(values.endDate);
