...
describe('Common substitute operating periods', () => {
  ...
  it(
    ...
    () => {
      ...
      const route1231InboundThursdayVehicleService = new VehicleServiceTable(
        route1231InboundTimetableSection,
        'TO',
      );

      const route1231InboundSaturdayVehicleService = new VehicleServiceTable(
        route1231InboundTimetableSection,
        'LA',
      );
      ...
      cy.visit(
        `timetables/lines/${lines[0].line_id}?observationDate=2024-12-26&routeLabels=${routes[0].label}`,
      );

      vehicleScheduleDetailsPage.getShowAllValidSwitch().click();

      route1231InboundThursdayVehicleService
        .get()
        .should('contain', '26.12.2024 - 26.12.2024')
        .and('contain', '2 lähtöä')
        .and('contain', '07:05 ... 08:20');
      route1231InboundSaturdayVehicleService
        .get()
      ...
      cy.wait('@gqlDeleteSubstituteOperatingPeriod')
        .its('response.statusCode')
        .should('equal', 200);
