...
import { TerminalDetailsPage } from '../../pageObjects';
...
describe('Terminal details', () => {
  ...
  const verifyInitialBasicDetails = () => {
    ...
    bdView.getNameEng().shouldHaveText('Terminal');
    bdView.getNameLongFin().shouldHaveText('Terminaali pitkänimi');
    bdView.getNameLongSwe().shouldHaveText('Terminalen långnamn');
    bdView.getNameLongEng().shouldHaveText('Terminal long name');
    bdView.getAbbreviationFin().shouldHaveText('Terminaali');
    bdView.getAbbreviationSwe().shouldHaveText('Terminalen');
    bdView.getAbbreviationEng().shouldHaveText('Terminal');
  ...
  const verifyInitialLocationDetails = () => {
    ...
    locationView.getMemberStopAreas().shouldHaveText('E2E008');
  };
