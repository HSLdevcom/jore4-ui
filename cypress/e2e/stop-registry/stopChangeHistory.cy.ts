...
describe('Stop Change History', { tags }, () => {
  ...
  describe('Value diffs', () => {
    ...
    it('Should diff Shelter Details', () => {
      ...
      cy.info('Wait for the UI to refresh on screen shelter data.');
      ShelterViewCard.getNthContainer(0).within(() => {
        ShelterViewCard.getShelterNumber().shouldHaveText('2');
      });
    ...
    it('Should diff Info Spot Details', () => {
      ...
      InfoSpotViewCard.getNthViewCardContainer(0).within(() => {
        InfoSpotViewCard.getLabel().shouldHaveText(
          changedInfoSpot.addedFirstInfoSpot.label[1],
        );
      });
  ...
  describe('Sorting', () => {
    ...
    function assertSectionsAreInOrder(
      ...
      // Make sure we are not displaying "previous items".
      // This is needed because the next Cypress command uses each to validate
      // the order, and Cypress does not retry each sections.
      StopChangeHistoryPage.changeHistoryTable
        .getTable()
