...
import { ChangeHistoryTableInfoSpotSectionHeader } from '../../pageObjects/base-change-history/ChangeHistoryTableInfoSpotPageObjects';
...
const changedOwnerDetails = {
  ownerName: 'Uusi omistaja',
  ownerPhone: '0401234567',
  ownerEmail: 'uusi.omistaja@example.com',
  contractId: 'NEW-CONTRACT-123',
  note: 'Uudet sopimuksen ehdot',
} as const;
...
describe('Terminal Change History', { tags }, () => {
  ...
  it('Should diff location and member stop details', () => {
    initTestData();
      ...
      TerminalChangeHistory.changeHistoryTable.group
        ...
        .within(() => {
          TerminalChangeHistory.changeHistoryTable.changedValues.terminalStops
            .getStops()
            .within(assertValueChanged(['H2003', ['H2003', 'V1562'].join('')]));
        });
  ...
  it('Should diff owner details', () => {
    ...
    cy.section('Make changes to owner', () => {
      ...
      ownerModal.form.getName().clearAndType(changedOwnerDetails.ownerName);
      ownerModal.form.getPhone().clearAndType(changedOwnerDetails.ownerPhone);
      ownerModal.form.getEmail().clearAndType(changedOwnerDetails.ownerEmail);
      ownerModal.form.getSaveButton().click();

      edit.getContractId().clearAndType(changedOwnerDetails.contractId);
      edit.getNote().clearAndType(changedOwnerDetails.note);
    ...
    cy.section('Check changed details', () => {
      ...
      TerminalChangeHistory.changeHistoryTable.group
        ...
        .within(() => {
          ownerDetails
            .getOwnerName()
            .within(assertValueChanged(['-', changedOwnerDetails.ownerName]));
          ownerDetails
            .getOwnerContractId()
            .within(assertValueChanged(['-', changedOwnerDetails.contractId]));
          ownerDetails
            .getOwnerNote()
            .within(assertValueChanged(['-', changedOwnerDetails.note]));
        });
    });
  });
        ...
        .within(() => {
          ...
          TerminalChangeHistory.changeHistoryTable.changedValues.terminalExternalLinks
            ...
            .within(() => {
              assertExternalLinksChanged(
                () =>
                  TerminalChangeHistory.changeHistoryTable.changedValues.getOldValue(),
                () =>
                  TerminalChangeHistory.changeHistoryTable.changedValues.getNewValue(),
      ...
      TerminalChangeHistory.changeHistoryTable.group
        ...
        .within(() => {
          terminalDetails
            ...
            .within(
              assertValueChanged([
                formatShortDate(initialValidityStart),
                formatShortDate(newStart),
              ]),
            );
          terminalDetails
            .getValidityEnd()
            .within(
              assertValueChanged([
                formatShortDate(initialValidityEnd),
