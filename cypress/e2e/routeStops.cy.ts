...
const buildStops = (infrastructureLinkIds: UUID[]): StopInsertInput[] => [
  // included on route
  {
    ...buildStop({
      label: stopLabels[0],
...
describe('Line details page: stops on route', () => {
  ...
  before(() => {
    let infraLinkIds: string[] = [];

    cy.task(
      'hasuraAPI',
      mapToGetInfralinksQuery(infrastructureLinkExternalIds),
    ).then((res) => {
      infraLinkIds = mapInfraLinkToIds(res);

      const stops = buildStops(infraLinkIds);
