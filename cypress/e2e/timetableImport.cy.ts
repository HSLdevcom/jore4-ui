...
describe('Timetable import', () => {
  ...
  context('Temporary timetables', () => {
    ...
    it(
      ...
      () => {
        ...
        const IMPORT_FILENAME = 'hastusImportSundayTemporary.exp';
  ...
  context('Draft timetables', () => {
    it(
      'Should export a route and import a Hastus timetable as a draft',
      { tags: [Tag.Timetables, Tag.HastusImport, Tag.HastusExport] },
      () => {
        ...
        // Check the imported timetable is not visible on the timetables page since it is a draft
        cy.visit(
          `timetables/lines/${lines[0].line_id}?observationDate=2023-05-06&routeLabels=${routes[0].label}`,
        );
        cy.contains('Ei vuoroja');
        ...
        timetableVersionsPage.timetableVersionTableRow
          ...
          .and('contain', 'Lauantai')
          .and('contain', '1.5.2023')
          .and('contain', '30.6.2043')
          .and('contain', '99');
      },
    );
  });
