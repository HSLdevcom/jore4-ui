...
describe('Timetable replacement and combination', () => {
  ...
  it(
    'Should import timetable data and replace existing data',
    ...
    () => {
      ...
      const IMPORT_FILENAME = 'timetablesReplace1.exp';

      // Skip searching via UI
      cy.visit('/routes/search?label=99&priorities=10&displayedType=routes');
      // Export the route to enable timebtale import for it
  ...
  it(
    'Should import timetable data and combine existing data',
    ...
    () => {
      ...
      const IMPORT_FILENAME = 'timetablesCombine1.exp';
