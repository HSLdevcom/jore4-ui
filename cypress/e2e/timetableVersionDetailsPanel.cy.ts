...
describe('Timetable version details panel', () => {
  ...
  it('Should open, have correct details, and close', () => {
    ...
    timetableVersionDetailsPanel.expandNthRow({ rowNumber: 0 });
    cy.getByTestId('ExpandableRouteTimetableRow::row')
      .eq(0)
    ...
    timetableVersionDetailsPanel.expandNthRow({ rowNumber: 1 });
    cy.getByTestId('ExpandableRouteTimetableRow::row')
      .eq(1)
      .findByTestId('VehicleServiceRow::row')
      .then((rows) => {
  ...
  it('Should change the validity period and update all correct validities', () => {
    ...
    timetableVersionsPage.timetableVersionTableRow
      .getRow()
      .eq(0)
      .findByTestId('TimetableVersionTableRow::row::validityEnd')
      .contains('31.3.2024');
  });
});
