...
describe('Hastus export', () => {
  ...
  context.only(
    'Fail cases, first and/or last stop are not used as timing points',
    () => {
      before(() => {
        cy.task<UUID[]>(
          'getInfrastructureLinkIdsByExternalIds',
      ...
      it(
        'should show an error, when the first stop is not an timing point',
        { tags: [Tag.Routes, Tag.HastusExport] },
        () => {
          ...
          routesAndLinesPage.toast.checkDangerToastHasMessage(
            'Seuraavia reittejä ei voida viedä: 901 (inbound). Ensimmäisen ja viimeisen pysäkin täytyy olla asetettuna käyttämään Hastus-paikkaa.',
          );
        },
      );

      it(
        'should show an error, when the last stop is not an timing point',
        { tags: [Tag.Routes, Tag.HastusExport] },
        () => {
          lineDetailsPage.visit(baseDbResources.lines[0].line_id);
      ...
      it(
        'should show an error, when neither the last stop nor the first stop is an timing point',
        { tags: [Tag.Routes, Tag.HastusExport] },
        () => {
          lineDetailsPage.visit(baseDbResources.lines[0].line_id);
