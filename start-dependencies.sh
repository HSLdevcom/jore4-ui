...
function seed_infrastructure_links {
  ...
  while ! $SUCCESS; do
    ...
    if [[ $(psql postgresql://dbadmin:adminpassword@localhost:6432/jore4e2e -AXqtc "SELECT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'infrastructure_network' AND tablename = 'infrastructure_link');") = "t" ]]; then
      echo "Schema found! Seeding infrastructure links..."
        psql postgresql://dbadmin:adminpassword@localhost:6432/jore4e2e < test-db-manager/src/dumps/infraLinks/infraLinks.sql; 
        SUCCESS=true
    fi
    sleep 2
  done
