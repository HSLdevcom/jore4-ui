...
function check_pinned_hasura {
  ...
  # Find latest image with "seed-data" tag from docker hub
  ...
  dockerHubTag="$(echo ${dockerHubImageList} | ${DOCKER_JQ} --raw-output 'first(.results[] | select(.name | startswith("seed-"))).name')"
